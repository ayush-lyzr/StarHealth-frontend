...
export const DataPreloaderProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const preloadAllData = async () => {
      ...
      try {
        const ragResponse = await axios.get('http://localhost:8000/api/rag/databases', {
          ...
          if (starHealthRagId) {
            try {
              const contentResponse = await axios.get(
                `http://localhost:8000/api/rag/content?rag_id=${starHealthRagId}`,
                { timeout: 30000 } // Increased to 30s for cold start
              )
              setPreloadedData(prev => ({
                ...prev,
      ...
      try {
        const configResponse = await axios.get('http://localhost:8000/api/agents/config', {
          timeout: 30000, // Increased to 30s for cold start
        })
